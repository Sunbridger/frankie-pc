...
function libConfig(api, { entry, name, formats, dest, sourceMap, stylePath, clean }) {
    ...
    const libName = name || api.service.pkg.name || path.basename(entry).replace(/\.(jsx?|vue)$/, '');
    ...
    function genConfig(format, postfix) {
        ...
        config.externals([nodeExternals(), ...(config.get('externals') || [])]);
